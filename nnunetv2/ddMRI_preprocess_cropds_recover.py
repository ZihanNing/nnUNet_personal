...
def save_preprocess_json(log_dict, json_path):
    """
    Save preprocessing metadata for later inverse recovery.
...
def preprocess(infile: str, outfile: str, mode: str, is_label: bool):
    ...
    json_path = outfile.replace(".nii.gz", ".json").replace(".nii", ".json")
    save_preprocess_json(log, json_path)

def recover_mask_to_original(pred_mask_nii, preprocess_json, out_nii):
    """
    Recover a predicted mask (from cropped + downsampled space)
    back to the original image space, zero-padding outside the crop.
    ...
    """
    Use case for recovery
    
    recover_mask_to_original(
        pred_mask_nii="case001_pred.nii.gz",
        preprocess_json="case001_prep.json",
        out_nii="case001_mask_recovered.nii.gz"
    )
    """
    import numpy as np
    import nibabel as nib
    ...
    pred = nib.load(pred_mask_nii).get_fdata()

    # ---------- undo final crop ----------
    crop1_shape = np.array(log["crop1"]["shape"])
    final_start = np.array(log["final_crop"]["start"])

    tmp = np.zeros(crop1_shape, dtype=pred.dtype)
    fe = final_start + np.array(pred.shape)
    tmp[
        final_start[0]:fe[0],
        final_start[1]:fe[1],
        final_start[2]:fe[2]
    ...
    # ---------- undo resampling ----------
    vox_before = np.array(log["resample"]["voxel_size_before"])
    vox_after = np.array(log["resample"]["voxel_size_after"])
    zoom_factor = vox_after / vox_before

    tmp_hr = zoom(tmp, zoom_factor, order=0)

    # ---------- embed into original image ----------
    orig_shape = np.array(log["original"]["shape"])
    crop_start = np.array(log["crop1"]["start"])

    out = np.zeros(orig_shape, dtype=pred.dtype)
    end = crop_start + tmp_hr.shape
    out[
        crop_start[0]:end[0],
        crop_start[1]:end[1],
        crop_start[2]:end[2]
    ...
    affine = np.array(log["original"]["affine"])
    nib.save(nib.Nifti1Image(out.astype(np.uint8), affine), out_nii)
